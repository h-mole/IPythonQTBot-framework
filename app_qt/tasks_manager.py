...
from PySide6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QSplitter,
                                ...
                                QLineEdit, QTextEdit, QDateTimeEdit, QFormLayout,
                                QDialogButtonBox, QCheckBox, QTimeEdit,
...
class TaskDialog(QDialog):
    ...
    def load_user_categories(self):
        """加载用户自定义的分类"""
        try:
            categories_file = os.path.join(os.path.dirname(DATA_PATHS['tasks_file']), 'task_categories.json')
            if os.path.exists(categories_file):
                with open(categories_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                    self.user_categories = data.get('categories', DEFAULT_CATEGORIES)
                    self.user_subcategories = data.get('subcategories', DEFAULT_SUBCATEGORIES)
            else:
    ...
    def init_ui(self):
        ...
        if self.task_data and self.task_data.get('reminder_time'):
            try:
                # 尝试从旧格式解析（包含日期）
                reminder_time = datetime.strptime(self.task_data['reminder_time'], "%Y-%m-%d %H:%M")
                ...
                try:
                    # 尝试从新格式解析（只有时间）
                    reminder_time = datetime.strptime(self.task_data['reminder_time'], "%H:%M")
                    time_obj = QTime(reminder_time.hour, reminder_time.minute)
                    self.reminder_time_edit.setTime(time_obj)
                except:
...
class TasksManagerTab(QWidget):
    ...
    # 表格列定义
    COLUMNS = ["ID", "任务大类", "任务小类", "任务说明", "任务完成日期", 
               "提醒方式", "提醒时间", "完成状态", "备注"]
    ...
    def __init__(self):
        ...
        self.categories_file = os.path.join(os.path.dirname(self.tasks_file), 'task_categories.json')
        self.tasks_data = []
        self.remind_timers = {}
    ...
    def init_ui(self):
        ...
        self.subcategory_filter = MultiSelectFilter("任务小类:", DEFAULT_SUBCATEGORIES.copy())
        ...
        header.setSectionResizeMode(1, QHeaderView.ResizeMode.ResizeToContents)  # 任务大类
        header.setSectionResizeMode(2, QHeaderView.ResizeMode.ResizeToContents)  # 任务小类
        header.setSectionResizeMode(7, QHeaderView.ResizeMode.ResizeToContents)  # 完成状态
        header.setSectionResizeMode(4, QHeaderView.ResizeMode.ResizeToContents)  # 任务完成日期
        header.setSectionResizeMode(5, QHeaderView.ResizeMode.ResizeToContents)  # 提醒方式
        header.setSectionResizeMode(6, QHeaderView.ResizeMode.ResizeToContents)  # 提醒时间
        ...
        try:
            if os.path.exists(self.categories_file):
                with open(self.categories_file, 'r', encoding='utf-8') as f:
                    ...
                    subcategories = data.get('subcategories', DEFAULT_SUBCATEGORIES)
                    # 更新筛选器的项目列表
    ...
    def save_tasks(self):
        ...
        try:
            ...
            # 写入表头
            ws.append(["ID", "任务大类", "任务小类", "任务说明", "任务完成日期",
                      "提醒方式", "提醒时间", "完成状态", "备注"])
    ...
    def refresh_table(self, filtered_data=None):
        ...
        for task in data_to_show:
            ...
            self.table.setItem(row_position, 0, QTableWidgetItem(str(task.get('id', ''))))
            self.table.setItem(row_position, 1, QTableWidgetItem(task.get('category', '')))
            self.table.setItem(row_position, 2, QTableWidgetItem(task.get('subcategory', '')))
            ...
            self.table.setItem(row_position, 5, QTableWidgetItem(task.get('reminder_type', '')))
            self.table.setItem(row_position, 6, QTableWidgetItem(task.get('reminder_time', '')))
    ...
    def sort_data(self, data):
        ...
        if sort_key == "categories":
            # 按类别排序：先按大类，再按小类
            sorted_data = sorted(data, key=lambda x: (x.get('category', ''), x.get('subcategory', '')), reverse=reverse)
        elif sort_key == "due":
            # 按截止期限排序
            def parse_date(task):
                due_date = task.get('due_date', '')
    ...
    def check_reminders(self):
            ...
            if reminder_type == "每天":
                ...
                if due_date:
                    try:
                        ...
                        if due_date_str == current_date_str and task_key not in self.notified_tasks:
                            should_remind = True
                            self.notified_tasks[task_key] = True
                    except:
                        pass
        ...
        # 清理过期的通知记录（保留今天的记录）
        self.notified_tasks = {k: v for k, v in self.notified_tasks.items() if current_date_str in k}
    ...
    def update_status(self):
        ...
        self.status_label.setText(f"总任务数：{total} | 未完成：{incomplete} | 已完成：{completed}")