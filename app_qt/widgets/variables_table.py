...
class VariablesTable(QWidget):
    ...
    def refresh_variables(self):
        ...
        try:
            ...
            # 需要排除的保留变量名
            reserved_names = {'In', 'Out', 'exit', 'quit', 'get_ipython', 
                              'open', 'input', 'help', '_', '__', '___'}
            ...
            for name, value in user_ns.items():
                ...
                if (not name.startswith('_') and 
                    name not in sys.modules and 
                    name not in reserved_names and
                    not callable(value) and
                    not isinstance(value, (types.FunctionType, types.MethodType, type))):
                    try:
